...
impl MirrorHost {
    ...
    pub fn rotate_token(&self) -> Result<MirrorStatus, String> {
        ...
        // Rebuild public URL with new token when we have a tunnel base without path token.
    ...
    fn status_unlocked(&self, g: &Inner) -> MirrorStatus {
        ...
        match &g.runtime {
            ...
            Some(r) => {
                let tail = r.token.chars().rev().take(6).collect::<String>().chars().rev().collect::<String>();
                MirrorStatus {
                running: true,
