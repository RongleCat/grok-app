...
pub async fn composer_prefs_set(
    ...
    if let Some(mid) = model_id {
        mgr.set_model(mid)
            .await
            .map_err(|e| format!("Could not switch model: {e}"))?;
    }
    if let Some(eff) = effort {
        mgr.set_effort_and_respawn_needed(&app, eff)
            .await
            .map_err(|e| format!("Could not switch reasoning effort: {e}"))?;
    }
    if let Some(m) = mode {
...
pub async fn session_set_model(
    ...
    mgr.set_model(model_id).await.map_err(|e| format!("Could not switch model: {e}"))?;
    Ok(prefs)
}
