 ...
 *
 * Never pass a raw catalog id to `Intl` directly: `fil` and `sa` have thin ICU
 * data and `zh` must resolve to `zh-CN`, not the generic macrolanguage.
 */
const INTL_TAGS: Readonly<Record<Locale, string>> = {
  ...
  ja: "ja-JP",
  ko: "ko-KR",
  // Latin has no CLDR calendar/number data — format dates and counts in English.
  la: "en-GB",
  "pt-BR": "pt-BR",
  ru: "ru-RU",
  // Sanskrit CLDR data is minimal; Hindi shares the script and numbering.
  sa: "hi-IN",
  ta: "ta-IN",
  uk: "uk-UA",
  zh: "zh-CN",
...
export function htmlLangForLocale(locale: Locale): string {
  ...
  // `la` and `sa` are real language subtags even where ICU data is thin — the
  // document language must stay honest for screen readers, unlike `intlLocale`.
  return locale;
}
