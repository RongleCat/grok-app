...
const en = {
  ...
  "app.versionFooter": "Grok v0.1.7 · MIT · Unofficial · Sister project grok-go",
...
"settings.prefsScope": "Remember model & permission at",
  ...
  "ext.mcp.note":
    "Add/remove writes the active agent config (`GROK_HOME` / agent-home). Toggles inject enabled servers into the next session after soft-respawn.",
  "ext.enabled": "Enabled",
  ...
  "ext.hooks.desc":
    "Lifecycle scripts under ~/.grok/hooks and the project .grok/hooks folder. List and open folders here; edit files in your editor.",
...
const zh: Record<MessageKey, string> = {
  ...
  "app.versionFooter": "Grok v0.1.7 · MIT · 非 xAI 官方 · 姐妹项目 grok-go",
...
"settings.prefsScope": "模型与权限记忆范围",
  ...
  "settings.wallpaperDesc":
    "上传自定义壁纸（JPEG/PNG/WebP、GIF 或 MP4/WebM）。静态图会压缩后保存在本地；视频和 GIF 会作为动态背景播放。",
  "settings.wallpaperEmpty": "尚未设置自定义背景",
  ...
  "ext.mcp.note":
    "添加/移除写入当前 agent 配置（`GROK_HOME` / agent-home）。开关会在 soft-respawn 后注入已启用的服务器到下一会话。",
  "ext.enabled": "已启用",
  "ext.disabled": "已禁用",
  "ext.enableAll": "全部启用",
  "ext.footnote":
    ...
    "ext.hooks.title": "Hooks",
  "ext.hooks.desc":
    "生命周期脚本位于 ~/.grok/hooks 与项目 .grok/hooks。此处可列表并打开目录；请用编辑器修改文件。",
...
"error.needTauri": "需要在 Tauri 窗口中选择目录",
  ...
  "settings.remoteIm.sidebarAria": "远程 IM 渠道",
