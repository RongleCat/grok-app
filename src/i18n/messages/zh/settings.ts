...
export const zhSettings = {
  ...
  "settings.streamStallSecondsDesc": "若一轮对话在该时间内无任何流式片段或工具活动，应用会先尝试静默收尾已真正结束的 Host 状态，再提示「继续等待 / 结束本轮」（默认 180）。仍有工具事件的长任务不会误判为卡顿。应用不会擅自结束你发起的任务——只有你点「结束本轮」才会停。",
  ...
  "settings.trayBusyBadgeDesc": "有 Agent 忙碌时，在 macOS 程序坞角标（或其他平台托盘提示）显示忙碌会话数。关闭则清除角标。",
  "settings.trayBusyBadge.status.off": "角标已关闭 — 程序坞/托盘计数已清除。",
  "settings.trayBusyBadge.status.idle": "当前无忙碌会话 — 角标保持空白。",
  "settings.trayBusyBadge.status.busy": "程序坞/托盘显示 {n} 个忙碌会话。",
  ...
  "settings.sessionModeHelp": "独立模式会话存在应用目录；共享模式使用 CLI 主目录，与终端共用会话。两种模式均可导入 CLI 会话；独立模式扫描的是 agent-home（不一定是 ~/.grok）。",
