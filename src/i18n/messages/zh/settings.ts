...
export const zhSettings = {
  ...
  "settings.cliUpdate.appBehindTitle": "请先更新 App 再升级 CLI",
  "settings.cliUpdate.appBehindMsg": "当前 App 为 {app}，已有 {latestApp}（CLI {cli} 建议 App ≥ {minApp}）。单独升级 CLI 可能导致兼容问题。请先更新 Grok App，或仍要继续。",
  "settings.cliUpdate.appBehindConfirm": "仍要继续",
  ...
  "settings.prefsScope": "权限与模式记忆范围",
  "settings.prefsScopeDesc": "全局：所有对话共用。项目：按工作区记忆。会话：仅当前聊天。模型与思考等级始终按会话记忆。",
  ...
  "settings.msgRailSide": "对话进度条位置",
  "settings.msgRailSideDesc": "在对话左侧或右侧显示进度刻度，用于在消息间跳转",
  "settings.msgRailSide.left": "左侧",
  "settings.msgRailSide.right": "右侧",
  ...
  "settings.modeIndependent": "独立（应用 agent-home）",
  "settings.modeShared": "共享（~/.grok）",
  "settings.sessionModeHelp": "默认为共享：App 与终端 Grok Build 共用 ~/.grok（同一会话列表）。独立模式隔离到应用 agent-home（应用数据目录，不一定是 ~/.grok-app）。两种模式均可导入 CLI 会话；独立模式扫描 agent-home（不一定是 ~/.grok）。",
  ...
  "settings.permissionRulesAddWorking": "正在保存…",
  "settings.permissionRulesDesc": "在 agent 的 config.toml 中精简管理 allow / deny / ask 模式（优先级：deny 高于 ask 高于 allow）。示例：Bash(git *)、Read(src/**)、Edit。改动会 soft-respawn Agent，使下一轮重新加载规则。",
  "settings.permissionRulesEmpty": "还没有规则 — 在下方添加一条模式。",
  ...
  "settings.ssh.configMissing": "{path} 没有 SSH 配置。在 ~/.ssh/config 里加 Host 别名后再刷新。",
  "settings.ssh.noHosts": "没有具体的 Host 别名。像 Host * 这种通配项会被忽略。",
  "settings.ssh.test": "测试连接",
  "settings.ssh.testing": "测试中…",
  "settings.ssh.statusSshOk": "SSH 正常",
  "settings.ssh.statusSshFail": "SSH 失败：{error}",
  "settings.ssh.statusCliOk": "Grok CLI {version}",
  "settings.ssh.statusCliMissing": "远程主机上没有找到 Grok CLI",
  "settings.ssh.statusAuthOk": "已登录",
  "settings.ssh.statusAuthMissing": "未登录",
  "settings.ssh.installTitle": "在远程主机安装 Grok CLI",
  "settings.ssh.installHint": "在本机复制运行。会在 {alias} 上开一个 login shell。",
