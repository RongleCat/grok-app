...
export const zhSettings = {
  ...
  "settings.streamStallSecondsDesc": "若一轮对话在该时间内无任何流式片段或工具活动，应用会先尝试静默收尾已真正结束的 Host 状态，再提示「继续等待 / 结束本轮」（默认 600 秒 = 10 分钟）。仍有工具事件的长任务不会误判为卡顿。应用不会擅自结束你发起的任务——只有你点「结束本轮」才会停。",
  ...
  "settings.backgroundWaitPolicyDesc": "无头首轮结束后（`grok -p`、远程 IM、类自动化路径）是否继续等待后台 bash/monitor 与后台子代理（CLI 0.2.117+）。默认等待（CLI 默认）。CLI 足够新时 ACP 启动也会传入顶层 flag；更旧 CLI soft-fail（省略 flag）。",
  ...
  "settings.backgroundWaitTimeoutDesc": "传入 `grok --background-wait-timeout N`（1–3600）。仅在策略为「限时等待」时使用。更改后 soft-respawn。",
  ...
  "settings.autoWakeDesc": "开启后，后台工作完成时（bash / monitor / 任务 / loop）Grok Build 可注入合成回合，无需用户再发消息即可继续处理。行为在 CLI 侧生效（配置 auto_wake_enabled，受支持时）。默认关闭。独立模式仅写入 agent-home config.toml——不编造 env 覆盖。更改后 soft-respawn；旧版 CLI 忽略该键时 soft-fail。",
  ...
  "settings.uiFontDesc": "主界面正文字体（侧栏、聊天、设置等）。留空使用系统默认。与终端字体分开配置。",
  "settings.uiFont.default": "系统默认",
  "settings.uiFont.custom": "自定义…",
  "settings.uiFont.family": "界面字体名称",
  "settings.uiFont.placeholder": "字体族名称（如 Inter、PingFang SC）",
  "settings.uiFont.preview": "The quick brown fox — 界面字体预览 Aa 123",
  "settings.terminalFont": "终端字体",
  "settings.terminalFontDesc": "内置终端使用的字体。请选择已安装的 Nerd Font，以便正确显示 Starship / Powerline 图标（不再是 □）。",
  "settings.terminalFont.default": "默认（Nerd 回退栈）",
  "settings.terminalFont.custom": "自定义…",
  "settings.terminalFont.family": "终端字体名称",
  "settings.terminalFont.placeholder": "例如 JetBrainsMono Nerd Font",
  "settings.terminalFont.size": "终端字号",
  "settings.terminalFont.hint": "请先在系统中安装 Nerd Font，再选择或输入准确的字体族名称。若图标仍为 □，可新开终端标签页后再试。",
