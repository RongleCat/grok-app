...
export const zhTWSettings = {
  ...
  "settings.experimentalMemoryDesc": "透過 --experimental-memory / GROK_MEMORY 讓 Grok Build 跨對話記住專案事實。實驗性功能，行為可能變化。預設關閉；關閉時 Agent 強制 --no-memory 以隔離。",
  ...
  "settings.streamStallSecondsDesc": "若一輪對話在該時間內無任何串流片段或工具活動，應用會先嘗試靜默收尾已真正結束的 Host 狀態，再提示「繼續等待 / 結束本輪」（預設 600 秒 = 10 分鐘）。仍有工具事件的長任務不會誤判為停滯。應用不會擅自結束你發起的任務——只有你點「結束本輪」才會停。",
  ...
  "settings.backgroundWaitPolicyDesc": "無頭首輪結束後（`grok -p`、遠端 IM、類自動化路徑）是否繼續等待背景 bash/monitor 與背景子代理（CLI 0.2.117+）。預設等待（CLI 預設）。CLI 足夠新時 ACP 啟動也會傳入頂層 flag；更舊 CLI soft-fail（省略 flag）。",
  ...
  "settings.backgroundWaitTimeoutDesc": "傳入 `grok --background-wait-timeout N`（1–3600）。僅在策略為「限時等待」時使用。變更後 soft-respawn。",
  ...
  "settings.autoWakeDesc": "開啟後，背景工作完成時（bash / monitor / 任務 / loop）Grok Build 可注入合成回合，無需使用者再發訊息即可繼續處理。行為在 CLI 側生效（設定 auto_wake_enabled，受支援時）。預設關閉。獨立模式僅寫入 agent-home config.toml——不編造 env 覆寫。變更後 soft-respawn；舊版 CLI 忽略該鍵時 soft-fail。",
  ...
  "settings.uiFont": "介面字體",
  "settings.uiFontDesc": "主介面正文字體（側欄、聊天、設定等）。留空使用系統預設。與終端字體分開設定。",
  "settings.uiFont.default": "系統預設",
  "settings.uiFont.custom": "自訂…",
  "settings.uiFont.family": "介面字體名稱",
  "settings.uiFont.placeholder": "字體族名稱（如 Inter、PingFang SC）",
  "settings.uiFont.preview": "The quick brown fox — 介面字體預覽 Aa 123",
  "settings.terminalFont": "終端字體",
  "settings.terminalFontDesc": "內建終端使用的字體。請選擇已安裝的 Nerd Font，以便正確顯示 Starship / Powerline 圖示（不再是 □）。",
  "settings.terminalFont.default": "預設（Nerd 回退棧）",
  "settings.terminalFont.custom": "自訂…",
  "settings.terminalFont.family": "終端字體名稱",
  "settings.terminalFont.placeholder": "例如 JetBrainsMono Nerd Font",
  "settings.terminalFont.size": "終端字號",
  "settings.terminalFont.hint": "請先在系統中安裝 Nerd Font，再選擇或輸入準確的字體族名稱。若圖示仍為 □，可新開終端分頁後再試。",
