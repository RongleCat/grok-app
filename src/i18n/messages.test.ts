...
describe("i18n catalog", () => {
  ...
  it("ships the same languages as the sibling Komorebi app", () => {
...
describe("resolveLocaleFromSystem", () => {
  ...
  it("maps the added OS languages to their catalog", () => {
    ...
    expect(resolveLocaleFromSystem("sa-IN")).toBe("sa");
    expect(resolveLocaleFromSystem("la")).toBe("la");
  });
...
describe("intlLocale / htmlLangForLocale", () => {
  ...
  it("routes catalogs with thin ICU data to a data-complete tag", () => {
    ...
    // Latin and Sanskrit have no usable CLDR calendar data of their own.
    expect(intlLocale("la")).toBe("en-GB");
    expect(intlLocale("sa")).toBe("hi-IN");
  });
  ...
  it("keeps html lang honest even where Intl data is borrowed", () => {
    ...
    // Screen readers must be told Latin, not the English we format dates with.
    expect(htmlLangForLocale("la")).toBe("la");
    expect(htmlLangForLocale("sa")).toBe("sa");
  });
