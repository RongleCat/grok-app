...
**中文 · 修复**
- **Windows 壁纸霜化会盖到侧栏和空会话**：WebView2 会跳过壁纸容器上的 `filter:blur`，停靠侧栏本身又是 `backdrop-filter: none`，半透明底下面的图还是清晰的。Windows 改为模糊 `<img>` / `<video>` 自己。Mac 不动。直播回合仍关掉模糊（`data-stream-perf`）。
