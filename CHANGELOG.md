...
- **Env dock is not a dropdown**: the card no longer uses `menu-panel` chrome; styles live in `sw-env-dock.css` so leftover dropdown rules cannot overlay the park motion.
...
- **环境信息钉板不再走下拉样式**：去掉 `menu-panel`，样式单独放在 `sw-env-dock.css`，避免旧下拉规则盖住 park 动画。
