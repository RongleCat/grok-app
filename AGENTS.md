...
2. Do **not** hardcode user-facing copy in any language. Use `createT(locale)` / `t()`. Thirteen locales ship (`de` `en` `es` `fr` `id` `it` `ja` `ko` `pt-BR` `ru` `uk` `zh` `zh-TW`); `en` is the key authority and every other catalog is English-backed. [i18n.md](docs/llm-wiki/i18n.md) is the source of truth for the shipped ids — do not add a locale without updating it. Never fork a date or number format on `locale === "zh"` — use `intlLocale()` / `isTightScript()`. See [i18n.md](docs/llm-wiki/i18n.md).
